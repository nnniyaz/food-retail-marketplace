...
type CatalogService interface {
	...
	Publish(ctx context.Context, catalog *catalog.Catalog, selectedSections map[string]*section.Section, selectedCategory map[string]*category.Category, selectedProducts map[string]*product.Product, slides []*slide.Slide, settings *orderSettings.OrderSettings) error
...
func (c *catalogService) Publish(ctx context.Context, catalog *catalog.Catalog, selectedSections map[string]*section.Section, selectedCategories map[string]*category.Category, selectedProducts map[string]*product.Product, slides []*slide.Slide, settings *orderSettings.OrderSettings) error {
	return c.catalogRepo.PublishCatalog(ctx, catalog, selectedSections, selectedCategories, selectedProducts, slides, settings)
}
