...
func NewCatalogService(l logger.Logger, repo repo.Catalog) CatalogService {
	return &catalogService{logger: l, catalogRepo: repo}
}
...
func (c *catalogService) Publish(ctx context.Context, catalog *catalog.Catalog) error {
	catalog.Publish()
	return c.catalogRepo.PublishCatalog(ctx, catalog)
}
