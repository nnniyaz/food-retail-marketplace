...
export const SectionActionCreators = {
    ...
    editSection: (sectionId: UUID, request: EditSectionReq, navigationCallback: NavigateCallback) => async (dispatch: AppDispatch, getState: () => RootState) => {
        const {currentLang} = getState().lang;
        try {
    ...
    recoverSection: (sectionId: UUID, navigationCallback: NavigateCallback) => async (dispatch: AppDispatch, getState: () => RootState) => {
        const {currentLang} = getState().lang;
        try {
            dispatch(SectionActionCreators.setIsLoadingRecoverSection(true));
            const res = await SectionService.recoverSection(sectionId);
            if (res.data.success) {
    ...
    deleteSection: (sectionId: UUID, navigationCallback: NavigateCallback) => async (dispatch: AppDispatch, getState: () => RootState) => {
        const {currentLang} = getState().lang;
        try {
